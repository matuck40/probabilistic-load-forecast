...
def calendar_frame(index: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    is_holiday = pd.Series([d.date() in dates for d in days], index=days)
    weekend = pd.Series(days.dayofweek >= 5, index=days)

    day_frame["is_holiday"] = is_holiday
    day_frame["is_holiday_eve"] = is_holiday.shift(-1, freq="D").reindex(days, fill_value=False)
    day_frame["is_day_after_holiday"] = is_holiday.shift(1, freq="D").reindex(days, fill_value=False)
    ...
    next_is_off = off.shift(-1, freq="D").reindex(days, fill_value=False)
    previous_is_off = off.shift(1, freq="D").reindex(days, fill_value=False)
