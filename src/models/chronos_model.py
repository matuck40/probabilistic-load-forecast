...
class ChronosForecaster(Forecaster):
    ...
    def predict(self, series: pd.Series, test_index: pd.DatetimeIndex) -> pd.DataFrame:
        ...
        for day in days:
            origin = day  # 00:00 of the forecast day
            history = series.loc[series.index < origin].to_numpy()[-self.context_length :]
            targets = test_index[(test_index >= origin) & (test_index < origin + pd.Timedelta(days=1))]
            ...
            # (see chronos/base.py and chronos/chronos.py). num_samples is forwarded
            # through **predict_kwargs to the sampling `predict` call underneath.
